...
const Grid = ({ diskRequests, cylinders }) => {
  ...
  useEffect(() => {
    ...
    const max = svg.clientWidth - 10; 

    const x = d3.scaleLinear().domain([0, cylinders]).range([10, max]); //Set top axis

    selection.append("g").style("color", "white").style("opacity", "1").call(d3.axisBottom(x)); //Append top axis
    ...
    for (let i = 0; i < diskRequests.length; i++) { //Draw points and push positions for later animating 
      ...
      const y = (((svg.clientHeight - 30) / diskRequests.length) * i) + 30; //+30 and -30 to make sure first horizontal line is below axis numbers
    ...
    const individualLines = createIndividualLines(_lines);

    setCircles(_circles);
    setLines(individualLines);
  ...
  return (
    // <div id="ref" className="h-full w-10/12 min-h-[500px] min-w-[700px] grow">
      <svg ref={ref} className="h-full w-10/12 min-h-[500px] min-w-[700px] grow">
        { 
          lines.map((line, i) => 
